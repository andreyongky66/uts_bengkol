...
if(isset($_POST['tambah'])) {
    ...
    $query = "INSERT INTO pasien (nama, alamat, no_hp) VALUES (?, ?, ?)";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param("sss", $nama, $alamat, $no_hp);
    
    if($stmt->execute()) {
        echo "<script>alert('Data berhasil ditambahkan!'); window.location='index.php?page=pasien.php';</script>";
    }
}
...
if(isset($_POST['update'])) {
    ...
    if($stmt->execute()) {
        echo "<script>alert('Data berhasil diupdate!'); window.location='index.php?page=pasien.php';</script>";
    }
}
...
if(isset($_GET['hapus'])) {
    ...
    if($stmt->execute()) {
        echo "<script>alert('Data berhasil dihapus!'); window.location='index.php?page=pasien.php';</script>";
    }
}
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form method="POST">
            ...
            if(isset($_GET['edit'])) {
                $id = $_GET['edit'];
                $edit_query = "SELECT * FROM pasien WHERE id=?";
            ...
            <tbody>
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>
                            ...
                            <a href='index.php?page=pasien.php&hapus=".$row['id']."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus?\")'>Hapus</a>
                          </td>";
                    echo "</tr>";
                }
                ?>
